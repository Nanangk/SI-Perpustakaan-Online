...
<style>
	.top-head{
		...
		background-color:rgb(60, 143, 41);
	}
...
<form style="margin-left:500px; width:15%; position: absolute; top:180px;" action="" method="post">
	<input style="text-align: center;"  type="text" name="keyword" size="30" placeholder="Masukkan Nama Event">
...
<div class="buku-koleksi-section">
	<div class="judul-buku">
		<div style="margin-bottom:-500px;" class="judul-koleksi">
			<p style="text-align: left;">Events</p>
			<div style="margin-top:5px; margin-left:4px;" class="gap3"></div>
		</div>
		<br><br><br><br><br><br>
		...
		//pagination
		 $perhalaman=10;
		 $cada=mysqli_query($conn,"SELECT * FROM event");
		...
		$datanya = "SELECT * FROM event ORDER BY hari_tgl DESC LIMIT $awal, $perhalaman";
		if(isset($_POST['cari'])){
  			$nama = $_POST['keyword'];
	...
	<div class="buku-koleksi">
		<div class="buku-item">
			...
			?>
				...
				<img  class="buku-gambar" src="proses/poster/<?php echo $data['poster']; ?>" width="120" heigth="160"  class="btn btn-info btn-xs view_data" >
    ...
    <div style="margin-top:-50px;" class="container">
      <h2 style="text-align: center; margin-bottom:20px; font-family: 'Yatra One';">Apakah anda yakin?</h2>
      	<div style="display: block; margin-left:40%;">
      		...
      		<a href="event.php"><button style="background-color:red" type="button"  class="buku-popup-baca">Tidak</button>
      	</div>
    </div>
  </form>
</div>
...
if(pageYOffset>0){
  ...
  drop[0].style.top="30px";
}
else{
	topNav[0].style.height="45px";
	midNav[0].style.height="50px";
	midNav[0].style.marginTop="-20px";
    drop[0].style.top="-200px";
}
