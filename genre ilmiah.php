...
<div class="buku-koleksi-section">
	<div class="judul-buku">
		<div class="judul-koleksi">
			<p style="text-align: left;">Ilmiah</p>
			<div style="margin-top:-10px; margin-left:2px;" class="gap3"></div>
		</div>
		...
		//pagination
		 $perhalaman=8;
		 $cada=mysqli_query($conn,"SELECT * FROM buku WHERE genre='ilmiah'");
		...
		$datanya = "SELECT * FROM buku WHERE genre='ilmiah' ORDER BY id DESC LIMIT $awal, $perhalaman";
		if(isset($_POST['cari'])){
  			$nama = $_POST['keyword'];
  			$datanya = "SELECT * FROM buku WHERE judul like '%$nama%' AND genre='ilmiah'";
  		}
  		$perbaris=4;
  		$i=0;
    ...
    <div style="margin-top:-50px;" class="container">
      <h2 style="text-align: center; margin-bottom:20px; font-family: 'Yatra One';">Apakah anda yakin?</h2>
      	<div style="display: block; margin-left:40%;">
      		...
      		<a href="genre ilmiah.php"><button style="background-color:red" type="button"  class="buku-popup-baca">Tidak</button>
      	</div>
    </div>
  </form>
</div>
